...
def forward(pi, A, B, O):
  ...
  Trans_B=B[:,O]
  for t in range(1,N):alpha[:,t]=np.multiply(Trans_B[:,t],np.matmul(alpha[:,t-1],A))
  ###################################################
  return alpha
...
def backward(pi, A, B, O):
  ...
  Trans_B=B[:,O]
  Tranpose_A=A.T
  for t in range(N-2,-1,-1):beta[:,t]=np.matmul(np.multiply(beta[:,t+1],Trans_B[:,t+1]),Tranpose_A)
  ###################################################
...
def seqprob_backward(beta, pi, B, O):
  ...
  ###################################################
  # Q3.2 Edit here
  prob= np.sum(np.multiply(beta[:,0],np.multiply(pi,B[:,O[0]])),axis=0)
  ###################################################
  return prob
...
def viterbi(pi, A, B, O):
  ...
  N = len(O)
  sigma=np.zeros((S,N))
  delta_s=np.zeros((S,N))
  Trans_B=B[:,O]
  sigma[:,0]=np.multiply(pi,B[:,O[0]])
  for t in range(1,N):
    Temp=np.multiply(np.expand_dims(sigma[:,t-1],axis=1),A)
    sigma[:, t]=np.multiply(Trans_B[:,t],np.max(Temp,axis=0))
    delta_s[:,t]=np.argmax(Temp,axis=0)
  #backTrack
  path=np.zeros(N,dtype=int)
  path[N-1] = np.argmax(sigma[:,N-1])
  for t in range(N-1,0,-1):
    path[t-1]=delta_s[path[t],t]
  path=path.tolist()
  ###################################################

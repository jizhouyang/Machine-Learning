...
def regularized_linear_regression(X, y, lambd):
  """
    ...
    """
  #####################################################
  w = np.matmul(np.matmul(np.linalg.inv(np.matmul(X.transpose(), X)+lambd*np.identity(X.shape[1])), X.transpose()), y)
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  #print(D)
  for a in lambds:
    w_test=regularized_linear_regression(Xtrain, ytrain, a)
    MSE=(np.square(np.linalg.norm(np.matmul(Xval,w_test)-yval)))/D;
    #print("lambd= ",a)
    #print("MSE= ", MSE)
    if bestlambda==None:
      bestlambda=a
...
def test_error(w, X, y):
  ...
  D=X.shape[0];
  #print (D)
  err = (np.square(np.linalg.norm(np.matmul(X, w) - y))) / D;
  return err

...
def compute_distances(Xtrain, X):
	...
	"""
	#####################################################
	Interme=(-2)*np.matmul(X,Xtrain.transpose())
	trainpart=np.square(Xtrain).sum(axis=1)
	dists=Interme+trainpart.reshape(1,Xtrain.shape[0])
	dists=np.sqrt(np.square(X).sum(axis=1).reshape(X.shape[0],1)+dists)
	del trainpart, Interme
	#####################################################
	return dists
...
def predict_labels(k, ytrain, dists):
	...
	def MyY(a):
		return ytrain[a]
	index1 = np.argpartition(dists, k, axis=1)
	index1=index1[:index1.shape[0], :k]
	yinterm=np.apply_along_axis(MyY,axis=1,arr=index1)
	yinterm=np.apply_along_axis(sorted,1,yinterm)
	ypred=np.apply_along_axis(myfunc, axis=1, arr=yinterm)
...
def compute_accuracy(y, ypred):
	...
	"""
	#####################################################
	Num_test=float(y.shape[0])
	temp=ypred-y
	#print(temp)
	acc=(Num_test-float(np.count_nonzero(temp)))/(Num_test)
	#print (acc)
	#####################################################
	return acc
...
def find_best_k(K, ytrain, dists, yval):
	...
	validation_accuracy=[]
	for a in K:
		temp =compute_accuracy(yval,predict_labels(a,ytrain,dists))
		validation_accuracy.append(temp)
		if best_k is None:
			best_k=a
			bestAcc=temp
		else:

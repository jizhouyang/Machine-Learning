...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for iter in range(0, max_iterations):
        temp=sigmoid(np.dot(X, w) + b) - y
        w =w- GFactor *  np.dot(x_v1,temp)
        b = b-GFactor * np.sum(temp)
...
def binary_predict(X, w, b):
    ...
    preds = np.asarray(sigmoid(np.dot(X, w) + b) > 0.5).astype(int)
    """
    TODO: add your code here
    """      
...
def multinomial_train(X, y, C, 
    ...
    for iter in range(max_iterations):
        temp=np.matmul(X, np.transpose(w)) + np.transpose(b)
        M = np.max(temp, axis=1).reshape(N,1)
        total = np.sum(np.exp(-M+temp), axis=1).reshape(N,1)
        y_union=np.transpose(np.array([np.asarray(y == Class).astype(int) for Class in range(C)]))
        Error=(np.exp(np.matmul(X, np.transpose(w)) + np.transpose(b)-M)/ total)-y_union
        w =w- (step_size / N) * (np.transpose(np.matmul(X.T,Error)))
        b =b- (step_size / N) * np.sum(Error,axis=0)
...
def multinomial_predict(X, w, b):
    ...
    preds = np.argmax(np.exp(np.matmul(X,w.T)+b.reshape(1,C)), axis=1)
    """
    TODO: add your code here
    """   
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    Class=0
    while Class<C:
        w[Class],b[Class]= binary_train(X, np.asarray(y == Class).astype(int),step_size=step_size,max_iterations=max_iterations)
        Class+=1
...
def OVR_predict(X, w, b):
    ...
    preds = np.argmax(sigmoid(np.matmul(X,np.transpose(w))+np.transpose(b)), axis=1)

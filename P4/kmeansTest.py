...
def transform_image(image, code_vectors):
    ...
    #Temp_data=image.reshape(image.shape[0]*image.shape[1],image.shape[2])
    s0=image.shape[0]
    s1=image.shape[1]
    s2=image.shape[2]
    s01=s0*s1
    ...
    return code_vectors[np.argmin(np.sum(((image.reshape(s01,s2) - np.expand_dims(code_vectors, axis=1)) ** 2), axis=2),axis=0)].reshape(s0, s1, s2)
